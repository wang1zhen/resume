\documentclass[10pt,a4paper]{article}

\usepackage[margin=1.5cm]{geometry}
\usepackage{iftex}
\usepackage{xcolor}
\usepackage{enumitem}
\usepackage{hyperref}
\usepackage{graphicx}
\usepackage{fontawesome5}
\usepackage{multicol}

\ifPDFTeX
  \usepackage[T1]{fontenc}
  \usepackage[utf8]{inputenc}
  \usepackage{lmodern}
\else
  \usepackage{fontspec}
  \setmainfont{Noto Sans CJK JP}
  \setsansfont{Noto Serif CJK JP}
  \renewcommand{\familydefault}{\sfdefault}
\fi

\definecolor{DarkInk}{HTML}{1F2937}
\definecolor{SteelGrey}{HTML}{4B5563}
\definecolor{LineGrey}{HTML}{D1D5DB}

\newcommand{\sectionrule}{\textcolor{LineGrey}{\rule{\linewidth}{0.6pt}}}
\newcommand{\cvsection}[1]{%
  \vspace{5pt}%
  {\Large\scshape #1}\par\vspace{-2pt}%
  \sectionrule\vspace{2pt}%
}
\newcommand{\contactitem}[2]{\textcolor{SteelGrey}{#1}\,#2}
\newcommand{\furiname}[4]{%
  \begin{tabular}[t]{@{}cc@{}}%
    {\large #1} & {\large #2}\\[-1pt]%
    {\Huge\bfseries #3} & {\Huge\bfseries #4}%
  \end{tabular}%
}
\newenvironment{cvitems}{\begin{itemize}[leftmargin=*,itemsep=4pt,topsep=2pt]}{\end{itemize}}
\newcommand{\eduentry}[5]{%
  \vspace{8pt}%
  \begin{tabular*}{\textwidth}{@{\extracolsep{\fill}} l r}
    \textbf{#1} & #4\\
    \textit{\small #2} & \textit{\small #3}\\
  \end{tabular*}%
  \if\relax\detokenize{#5}\relax
    \vspace{4pt}%
  \else
    \vspace{-2pt}%
    \begin{cvitems}
      #5
    \end{cvitems}
  \fi
}

\setlength{\parindent}{0pt}
\setlength{\parskip}{4pt}
\setlist[itemize]{leftmargin=*,itemsep=4pt,topsep=2pt,label={$\circ$}}

\hypersetup{
  colorlinks=true,
  linkcolor=DarkInk,
  urlcolor=DarkInk
}

\begin{document}
\thispagestyle{empty}

\begin{minipage}[b]{0.72\linewidth}
  \furiname{オウ}{イシン}{王}{一臻}\\[4pt]
  {\normalsize WANG\;Yizhen}

  \vspace{8pt}
  {\footnotesize
    \textcolor{DarkInk}{\normalsize 東京大学 長谷川研究室 修士}\\[2pt]
    \contactitem{\faEnvelope[regular]}{\href{mailto:wangyz@iis.u-tokyo.ac.jp}{wangyz@iis.u-tokyo.ac.jp}}\hspace{1em}
    \contactitem{\faPhone}{080-8008-2338}\hspace{1em}
    \contactitem{\faMapMarker*}{日本・東京}\\[2pt]
    \contactitem{\faGlobe}{\href{https://blog.wang1zhen.com}{blog.wang1zhen.com}}\hspace{1em}
    \contactitem{\faGithub}{\href{https://github.com/wang1zhen}{github.com/wang1zhen}}\hspace{1em}
    \contactitem{\faLinkedin}{\href{https://www.linkedin.com/in/\%E4\%B8\%80\%E8\%87\%BB-yizhen-\%E3\%82\%A4\%E3\%82\%B7\%E3\%83\%B3-\%E7\%8E\%8B-wang-\%E3\%82\%AA\%E3\%82\%A6-3a7b68110/}{LinkedIn}}
  }
\end{minipage}
\hfill
\begin{minipage}[b]{0.22\linewidth}
  \raggedleft
  \raisebox{6pt}{\includegraphics[width=3.2cm]{1.jpg}}
\end{minipage}

\cvsection{学歴}

\eduentry
  {東京大学}
  {修士課程（機械工学専攻、長谷川研究室）}
  {日本・東京}
  {2023.10 -- 2025.9}
  {\item 円管乱流実験における抵抗低減のための脈動波形の自動最適化
   \item 修士論文が日本自動車技術会（JSAE）の Award of Society of Automotive Engineers of Japan を受賞}

\eduentry
  {メルボルン大学}
  {修士課程（機械工学専攻、中退）}
  {オーストラリア・メルボルン}
  {2019.1 -- 2019.12}
  {\item 粗面高レイノルズ数乱流境界層におけるエネルギー運動の解明}

\eduentry
  {上海交通大学}
  {学士課程（エネルギー・動力工学専攻、音楽学副専攻）}
  {中国・上海}
  {2014.9 -- 2018.9}
  {\item DMEおよびエタノールによるメタン火炎の希釈効果
   \item 水素冷却発電機の固定子基礎の動特性}

\eduentry
  {上海市実験学校}
  {高等学校卒業}
  {中国・上海}
  {2011.9 -- 2014.8}
  {}

\begin{multicols}{2}
\cvsection{スキル}
\begin{itemize}[itemsep=6pt,topsep=2pt]
  \item \textbf{プログラミング}\\
        Python, MATLAB, C/C++, Emacs Lisp
  \item \textbf{3Dモデリング・シミュレーション}\\
        SolidWorks, OpenFOAM, ANSYS
  \item \textbf{ツール}\\
        Microsoft Office, \LaTeX
  \item \textbf{システム}\\
        GNU/Linux, openWRT, BSD
\end{itemize}

\columnbreak
\cvsection{語学力}
\begin{itemize}
  \item 中国語：母国語
  \item 英語：ネーティブレベル
  \item 日本語：JLPT N2（ビジネスレベル）
\end{itemize}

\cvsection{趣味}
\begin{itemize}[itemsep=4pt,topsep=2pt]
  \item クラシック音楽：アマチュアビオラ奏者
  \item パソコンDIY：サーバー管理者
  \item 推理小説
\end{itemize}
\end{multicols}

\end{document}
